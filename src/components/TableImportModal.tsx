...
import { downloadPlannerImportTemplate, importCalendarEventsFromTableFile } from '../lib/tableImport'
import type { CalendarImportResult } from '../lib/importUtils'

interface Props {
  onClose: () => void
}

export function TableImportModal({ onClose }: Props) {
  ...
  useEffect(() => {
    ...
    importCalendarEventsFromTableFile(selectedFile, store.events, store.categories, selectedCategory)
      .then((result) => {
        if (!cancelled) {
          setSummary(result)
        }
      ...
      .catch((importError) => {
        if (!cancelled) {
          setSummary(null)
          setError(importError instanceof Error ? importError.message : 'Could not read this spreadsheet.')
        }
      })
      .finally(() => {
        if (!cancelled) {
  ...
  }, [selectedCategory, selectedFile, store.categories, store.events])
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="table-import-title"
        style={{
          background: '#0d1224',
        ...
        >
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: '#94a3b8' }}>
              Upload a CSV or Excel file. This is ideal for schedules copied from Word or PDF into a spreadsheet.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-5">
          <div className="space-y-3">
            ...
            <div
              ...
              style={{ background: '#111827', border: '1px solid #1e2d40', color: '#94a3b8' }}
            >
              Use the STRATUM template headers: <span className="text-slate-200">Date, Title, Category, Start Time, End Time, Notes, Recurrence, Reminder</span>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                style={{ background: '#111827', color: '#d4af37', border: '1px solid #243447' }}
              >
            ...
            >
              ...
              <div>
                <p className="text-sm font-semibold text-white">Choose a CSV or Excel file</p>
                <p className="text-xs text-slate-500 mt-1">We will preview what can be imported before anything is saved.</p>
              </div>
              <span
          ...
          {summary && !completedCount && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: 'Rows', value: summary.totalFound },
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    {previewEvents.map((event) => {
                      const categoryLabel = store.categories.find((category) => category.id === event.category)?.label ?? event.category
                      return (
                        <div
                        ...
                        >
                          ...
                          <p className="text-xs text-slate-500 mt-1">
                            {event.date}{event.startTime ? ` at ${event.startTime}` : ' (all day)'} - {categoryLabel}
                          </p>
                        </div>
                      )
                    })}
          ...
          {completedCount !== null && summary && (
            ...
            >
              ...
              <p className="text-xs text-slate-400">
                {summary.duplicateCount} duplicate{summary.duplicateCount === 1 ? '' : 's'} skipped, {summary.unsupportedRecurrenceCount} row{summary.unsupportedRecurrenceCount === 1 ? '' : 's'} used one-time recurrence, {summary.skippedCount} row{summary.skippedCount === 1 ? '' : 's'} skipped.
              </p>
            </div>
          )}
        </div>
